...
项目的版本号应该根据某些规则进行迭代, 这里推荐使用[语义化版本](https://semver.org/lang/zh-CN/)规范, **通过这个规范，开发者可以了解版本变更的影响范围**。 规则如下:
...
必要和适量的注释对阅读源代码的人来说，就是一个路牌, 可以少走很多弯路.
...
前后端团队经过长期的合作，一般可以总结出一条对双方开发效率最优的协作流程. 这个可以落实成为规范，然后面的团队成员都遵循这个步调进行协作。
...
- 明确区分是正常还是异常, 严格遵循接口的异常原语. 比如上述接口形式都有明确的异常原语，比如JSONRPC，当出现异常时应该返回`错误对象`响应，而不是在正常的响应体中返回错误代码. 另外要规范化的错误码, HTTP响应码就是一个不错的学习对象
