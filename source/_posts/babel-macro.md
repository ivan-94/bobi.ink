...
- **首先Babel Macro必须是合法的Javascript语法**。不支持自定义语法，也要分两面讨论，合法的Javascript语法不至于打破现有的工具协作链，比如可以和ESLint、Typescript正常的配合, 如果允许用户毫无限制地创建新的语法，将来指不定会和标准的语法发生歧义。
  而不能自定义语法的‘宏’，显得不太地道，不够'强大'。
- **因为必须是合法的Javascript语法，Babel Macro 实现DSL(Domain-specific languages)就弱化了，DSL通常以一种简洁、声明式的方式来描述一些领域问题**
...
创建 Macro 文件. 按照上一节的介绍，我们使用`createMacro`来创建一个 `Macro`, 并从`references` 中拿出所有`导出标识符`的引用路径, 接着就是对这些引用路径进行AST转换:
...
t.evaluate(parse("foo + foo")) // { confident: false, value: undefined } ❌两个变量相加无法求值，因为变量值在运行时才存在，这里confident为false：  
```

所以如果这样子使用标签模板字符串是无法求值的:
...
所以，上面的代码还不够健壮，我们再优化一下，在求值失败时给用户更好的提示:
...
Babel Macro 本质上还是Babel 插件，只不过它是模块化的，你要使用它必须显式地导入。和‘正统’宏相比， Babel Macro 直接操作 AST，需要你掌握编译原理， ‘正统’宏可以实现的东西Babel Macro也可以实现(例如卫生宏). 虽然相比Babel插件略有简化，还是比较啰嗦。另外Babel Macro 不能创建新的语法，这使得它可以和现有的工具生态保持兼容。

最后！打开脑洞 🧠，Babel Macro 可以做很多有意思的东西，可以查看[《Awesome babel macros》](https://github.com/jgierer12/awesome-babel-macros)。不过要**谨记：‘显式好于隐式，清晰的代码优于简洁的代码’**
